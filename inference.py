...
def msa_generate(args, model, dataset, msa_collator, tokenizer):
    ...
    with torch.no_grad():
        output_dir = os.path.join(
            ...
            args.dataset,
            args.mode,
            f"A{args.augmentation_times}T{args.trials_times}R{args.repetition_penalty}",
        )
        args_dict = vars(args)
        ...
        for a3m_data in dataset:
            ...
            for trial in range(args.trials_times):
                ...
                generate_seq = [
                    tokenizer.decode(seq_token, skip_special_tokens=True).replace(" ", "") for seq_token in output[0]
                ]
...
def parsing_arguments():
    ...
    parser.add_argument(
        "--checkpoints", type=str, help="the folder path of model checkpoints, e.g '/msat5-base/checkpoint-740000'"
    )
    parser.add_argument("--data_path", type=str, help="the folder path of poor data eg. './dataset/casp15/cfdb/'")
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["orphan", "artificial"],
        required=True,
        help="whether task is real world orpha enhancement or artificial enhancement",
    )
    parser.add_argument("--repetition_penalty", type=float, default=1.2, help="repetition penalty for generation")
    parser.add_argument(
        "-a", "--augmentation_times", type=int, default=1, help="times of generated quality compared to original msa x1 x3 x5"
    )
