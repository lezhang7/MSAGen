...
class MSADataSet(Dataset):
    def __init__(self, data_dir=None, data_path_list=None, only_poor_msa=None):
        ...
        self.msa_data = {
            msa_file_path.split("/")[-1].split(".")[0]: self.read_msa(msa_file_path, 9999) for msa_file_path in data_path_list
        }
        assert all(
            [self.check_same_len(msa) for msa in self.msa_data.values()]
        ), "all sequence in a msa should have the same length"
        if only_poor_msa:
