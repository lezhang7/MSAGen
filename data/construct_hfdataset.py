...
def read_msa(filename: str, nseq: int) -> List[Tuple[str, str]]:
    ...
    return [
        (record.id, remove_insertions(str(record.seq))) for record in itertools.islice(SeqIO.parse(filename, "fasta"), nseq)
    ]
...
def main(args):
    ...
    msa_hfdataset = Dataset.from_dict(
        {"src": read_batch(msa_files), "tgt": read_batch(msa_files, "tgt"), "length": [len(msa[0][1]) for msa in msa_files]}
    )
    # print(msa_hfdataset[0])
    msa_hfdataset = msa_hfdataset.filter(
        lambda x: (len(x["src"]) == args.num_seq_per_msa) & (len(x["tgt"]) == args.num_seq_per_msa)
    )
