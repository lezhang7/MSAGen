...
class BatchConverter(object):
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]):
        ...
        seq_encoded_list = [
            self.tokenizer(self._tokenize(seq_str), truncation=True, max_length=self.max_len + 1).input_ids
...
if __name__ == "__main__":
    ...
    if args.random_src:
        print(
            "src_seq_per_msa will random select in {}-{}, total seq per msa is {}".format(
                args.src_seq_per_msa_l, args.src_seq_per_msa_u, args.total_seq_per_msa
            )
        )
